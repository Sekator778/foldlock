...
use foldlock::{
    compress, decompress, CompressOptions, CompressSummary, DecompressOptions, DecompressSummary,
};
...
EXAMPLES:
    foldlock compress ./photos s3cret 100      # 100 MiB volumes
...
fn run() -> Result<()> {
    ...
    match command {
        "compress" => {
            ...
            run_compress(&positionals)
        }
        "decompress" => run_decompress(&positionals, force),
        other => bail!("unknown command '{other}' (try 'foldlock --help')"),
    }
}

fn run_compress(positionals: &[String]) -> Result<()> {
    ...
    report_compress(&summary, size_mib);
    Ok(())
}
...
fn report_compress(summary: &CompressSummary, size_mib: u64) {
    println!(
